        ...
        if os.path.exists(transcripts_path):
            with open(transcripts_path, "r") as f:
                ...
                for i in range(0, len(lines), 2):
                    if i + 1 < len(lines):  # Check if there are enough lines for both title and transcript
                        st.subheader(lines[i].strip())
                        st.write("Transcript:", lines[i + 1].strip())
...
if options: 
    ...
    with col2: 
        ...
        # Store the predicted transcript in "Your Transcripts" section
        if st.button("Save Transcript"):
            with st.spinner("Saving Transcript..."):
                with open("transcripts.txt", "a") as f:
                    f.write(f"Video Title: {selected_video}\n")
...
if selected_language != 'English':
    translated_text = translate_to_language(predicted_text, selected_language)
    st.text(f"Translated to {selected_language}: {translated_text}")
